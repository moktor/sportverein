        ...
        if (request.method == 'POST'){
            ...
            def results = c {
                and {
                    ...
                    altersklassehinzufüegen {
                        like("id", ak)
                    }
                    min("sortierWert")                   
                }
                if (az) maxResults(az)
                order("sortierWert", params.order ? params.order : "desc")
                projections {
                    distinct("sportartAuswaehlen")
                    property("id")
                    property("sportlerHinzufuegen")
                    property("turnierHinzufuegen")
                    property("ergebnisEintragen")
                    property("sortierWert")
                }
            }
            [ergebnisInstanceList: results]
        }
    }
